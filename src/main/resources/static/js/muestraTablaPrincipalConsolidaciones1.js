$(document).ready(function () {
    // Asignar el evento al botón "Buscar"
    ...
    $("#idBtnBuscar").click(async function () {
        ...
        try {
            ...
            const response = await fetch(`/api/boletas/consultaPorCmmRangoFecha1?fecha_desde=${id_fechadesde}&fecha_hasta=${id_fechahasta}`);
    ...
    $('#id_busca_cmm').on('keyup', async function () {
        ...
        try {
            // Realizar la solicitud asíncrona con fetch
            const response = await fetch(`/api/boletas/consultaPorCmmRangoFecha1?cmm=${id_busca_cmm}`);
            const data = await response.json();
            agregarGrilla(data); // Función para actualizar la tabla con los datos recibidos
        } catch (error) {
            console.error('Error fetching data:', error);
...
//--------------------------------------------------------------------------------------------------
 ...
 async function cargarTodosLosRegistros() {
        try {
            ...
            const response = await fetch(`/api/boletas/consultaPorCmmRangoFecha1`);
            const data = await response.json();
            agregarGrilla(data); // Función para actualizar la tabla con los datos recibidos
        } catch (error) {
            console.error('Error fetching data:', error);
...
function agregarGrilla(lista){
  ...
  $('#tableGeneral').DataTable({
        ...
        render: function(data, type, row) {
          ...
          },
          {
              ...
              render: function(data, type, row) {
                  ...
                  var html = '';
                  if (botonConsolidacion == 1) {
                      html += 'ESTADO <button type="button" data-toggle="modal" onclick="cambiarDeEstado1a2(\'' + row.idBoleta + '\');" class="btn btn-active btn-xs" style="background-color: transparent; border: none;">';
      ...
      rowCallback: function(row, data) {
          $(row).removeClass('celeste rojo verde rojo_claro verde_claro');

          if (data.botonConsolidacion == 1) {
              $(row).addClass('rojo');

function cambiarDeEstado2a3(id) {
    // Mostrar un cuadro de confirmación antes de continuar
    var confirmar = confirm('Estas seguro de que deseas devolver a Entel el CMM, una vez devuelto , no podra ser modificado.');

    // Si el usuario hace clic en "Aceptar", continúa con la función
    if (confirmar) {
        $.ajax({
            url: '/guardaConsolidacion2a3?id=' + id, // Asegúrate de que el parámetro id se incluye correctamente
            type: 'PUT',
            success: function (data) {
                alert('EL CMM FUE devuelto CORRECTAMENTEE A Entel');
                recarga1();
        ...
        alert('La devolucion fue cancelada.');
    }
}


function cambiarDeEstado2a4(id) {
    // Mostrar un cuadro de confirmación antes de continuar
    ...
    if (confirmar) {
        $.ajax({
            url: '/guardaConsolidacion2a4?id=' + id, // Asegúrate de que el parámetro id se incluye correctamente
            type: 'PUT',
            success: function (data) {
                alert('EL CMM FUE CONSOLIDADO CORRECTAMENTE, EL CMM YA NO PUEDE SER MODIFICADO. Póngase en contacto con su administrador si cree que hubo algún error al registrar.');
                recarga1();

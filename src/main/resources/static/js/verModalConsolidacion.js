...
function habilitarcampos(boletas){
	...
	if(boletas.botonConsolidacion===1 || boletas.botonConsolidacion===3){
  ...
  try {
    ...
    if (request.ok) {
           alert("Material actualizado correctamente.");
			...
			await copiaMaterialLosCamposAcumuladoryDescripcionABoleta2(idBoleta);
			await actualizaElPrecioFinal3(idBoleta);
			//location.reload();
    } else {
...
async function copiaMaterialLosCamposAcumuladoryDescripcionABoleta2(idBoleta) {
    try {
...
async function actualizaElPrecioFinal3(idBoleta) {
    try {
